# TODO make buy sell distinction between stocks purchased for daytrading and long term purchases
'''
...
def daytrading_stock_analyzer(stocks):
    '''
    Purchases stocks based on daytrading patterns
...
def news_stock_analyzer(stock_ticker):
    '''
    Purchases stocks based on news info
    :param stock_ticker: stock ticker to check
    '''
    try:
        stock_score = 0
...
def stock_position_analyzer():
    '''
    Checks if any stocks should be sold based on daytrading patterns
    '''
    while True:
...
def check_perform_sell(stock_ticker, purchase_price):
    '''
    Checks if a stock should be sold based on daytrading patterns
    :param stock_ticker: stock ticker to check
    :param purchase_price: price stock was purchased at
    '''
    while True:
...
if __name__ == "__main__":
    ...
    for position in positions: #TODO also add orders
        active_positions_to_check[position.symbol] = float(position.cost_basis) # TODO cost basis not working well

    all_active_positions = active_positions_to_check.copy()
    print("Currently Purchased:", active_positions_to_check)
    first_time_run = True

    # TODO fix while loop timing to start running at stock market open time and stop at stock market close time
    while True:
        try:
